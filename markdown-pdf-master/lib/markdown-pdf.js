...
module.exports = {
  ...
  activate: function() {
    ...
    // }
    atom.config.set("mr-markdown-pdf.syntax" , atom.themes.getActiveThemeNames()[0]);
  },
  ...
  convert: function() {
    try{
      ...
      atom.notifications.addError('mr-markdown-pdf: ' + err, {dismissable: true});
...
var converter = {
  ...
  styleHtml: function (callback) {
    ...
    var styles = {};
    var syntaxName = atom.config.get("mr-markdown-pdf.syntax");
...
function makePdf(inputHtml, outputPath){
  ...
  var conf = atom.config.get('mr-markdown-pdf');
...
function getOutputPath(){
  ...
  if (!isMd(markdownPath)){    //file is saved as something other than markdown-type file
    ...
    var d = new Date();
    var outName = d.getTime() + '.' + atom.config.get('mr-markdown-pdf.type');
    var outputPath = path.join(treeDirPath, outName);
    return outputPath;
  }
  else{
    var out = markdownPath.replace(markdownPath.split(path.sep).slice(-1)[0], markdownPath.split(path.sep).slice(-1)[0].replace(getExtension(markdownPath), atom.config.get('mr-markdown-pdf.type')));
    return out;
  }
}
